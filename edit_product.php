...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($name_err) && empty($price_err)){
        ...
        $sql = "UPDATE products SET name = ?, price = ?, description = ?, image_url = ?, category = ?, color = ?, size = ? WHERE id = ?";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sdsssssi", $param_name, $param_price, $param_description, $param_image_url, $param_category, $param_color, $param_size, $product_id);
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                // For now, just redirect to manage_products
                header("location: manage_products.php");
                exit();
            }
        } else{
...
<body>
    ...
    <main class="admin_main">
        <div class="admin_header">
            <h1 class="admin_title">Upraviť produkt</h1>
        </div>
        ...
        <div class="admin_form_container">
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>?id=<?php echo $product_id; ?>" method="post">
                <div class="form-group <?php echo (!empty($name_err)) ? 'has-error' : ''; ?>">
                    <input type="text" name="name" class="auth_input" placeholder="Názov" required value="<?php echo $name; ?>">
                ...
                <div class="form-group">
                    <textarea name="description" class="auth_input admin_textarea" placeholder="Popis"><?php echo $description; ?></textarea>
                </div>
                <div class="form-group">
                    <input type="text" name="image_url" class="auth_input" placeholder="URL obrázka" value="<?php echo $image_url; ?>">
                </div>
                <div class="form-group">
                    <input type="text" name="category" class="auth_input" placeholder="Kategória" value="<?php echo $category; ?>">
                </div>
                <div class="form-group">
                    <input type="text" name="color" class="auth_input" placeholder="Farba" value="<?php echo $color; ?>">
                </div>
                <div class="form-group">
                    <input type="text" name="size" class="auth_input" placeholder="Veľkosť (oddelené čiarkou)" value="<?php echo $size; ?>">
                </div>
                
                <button type="submit" class="auth_btn">Uložiť zmeny</button>
